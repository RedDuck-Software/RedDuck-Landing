  ...
  return (
    ...
    >
      ...
      <div className="main-article-right">
        ...
        />
        <div className="main-article-right-title">{props.article.title}</div>
        <div className="main-article-right-desc">
          {`${moment(props.article.publish).format('DD.MM.YYYY')} | ${
        ...
        <div className="main-article-right-share">
          <div className="main-article-right-share-left">
            ...
            <a
              target="_blank"
              rel="noreferrer"
              href={`http://www.facebook.com/share.php?u=${props.article.link}`}
              onClick={() => {
                window.open(
                  `http://www.facebook.com/share.php?u=${props.article.link}`,
                  'newwindow',
                  `width=500,height=800`,
                );
              }}
              className="main-article-right-share-left-icon"
            >
              <FacebookIcon />
            </a>
            <a
              target="_blank"
              rel="noreferrer"
              href={`https://twitter.com/intent/tweet?text=${props.article.title}Article by @RedduckDev ${props.article.link}`}
              onClick={() => {
                window.open(
                  `https://twitter.com/intent/tweet?text=${props.article.title}%C2%A0Article by @RedduckDev ${props.article.link}`,
                  'newwindow',
                  'width=500,height=800',
                );
              }}
            ...
            <a
              target="_blank"
              rel="noreferrer"
              href={`https://www.linkedin.com/cws/share?url=${props.article.link}`}
              onClick={() => {
                window.open(
                  `https://www.linkedin.com/cws/share?url=${props.article.link}`,
                  'newwindow',
                  'width=500,height=800',
                );
              }}
